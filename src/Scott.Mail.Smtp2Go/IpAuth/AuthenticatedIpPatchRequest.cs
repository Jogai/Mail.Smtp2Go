...
/// <summary>The body of <c>PATCH /ip_auth/edit</c>: a partial edit of an authenticated IP. Omitted fields are left unchanged. <c>subaccount_id</c> is injected from <see cref="RequestOptions.SubaccountId"/>.</summary>
[Smtp2GoEndpoint("ip_auth/edit")]
public sealed record AuthenticatedIpPatchRequest : IRequestValidator
{
    /// <summary>The address of the existing entry to change.</summary>
    [JsonPropertyName("ip_address")]
    public required string IpAddress { get; init; }

    /// <summary>A comment or description of the entry.</summary>
    ...
    /// <summary>An address to BCC on every email sent from the IP.</summary>
    ...
    void IRequestValidator.Validate(Endpoint endpoint, ICollection<string> errors)
    {
        ...
        if (IpAddress is null || IpAddress.Trim().Length == 0)
        {
            errors.Add("ip_address is required.");
        }
        else if (!IPAddress.TryParse(IpAddress.Trim(), out _))
        {
            errors.Add("ip_address must be a valid IP address.");
        }
