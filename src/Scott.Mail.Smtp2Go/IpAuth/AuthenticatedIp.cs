...
/// <summary>
/// One authenticated IP as returned by <c>ip_auth/view</c> (wrapped in <c>{"results": [...]}</c>) and <c>ip_auth/edit</c> (a bare array);
/// <see cref="Json.Converters.ResultsListConverter{TItem}"/> reads both. The settings mirror those of an SMTP user.
/// </summary>
public sealed record AuthenticatedIp
{
    /// <summary>The IP address.</summary>
    [JsonPropertyName("ip_address")]
    public string? IpAddress { get; init; }

    /// <summary>The entry's description.</summary>
    ...
    /// <summary>The account's default rate-limit value, where returned per entry.</summary>
    [JsonPropertyName("default_ratelimit_value")]
    public int? DefaultRateLimitValue { get; init; }

    /// <summary>The account's default rate-limit period, where returned per entry.</summary>
    [JsonPropertyName("default_ratelimit_period")]
    public string? DefaultRateLimitPeriod { get; init; }

    /// <summary>The dedicated IP pool the entry sends from (<c>ippool</c> on the wire; <c>ip_pool</c> in requests).</summary>
    ...
    /// <summary>The address BCC'd on every email sent from the IP.</summary>
