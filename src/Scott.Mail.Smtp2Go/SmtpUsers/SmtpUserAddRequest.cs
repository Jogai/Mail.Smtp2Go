...
/// <summary>The body of <c>POST /users/smtp/add</c>. Only <see cref="Username"/> is required; the docs give the server defaults for the rest. <c>subaccount_id</c> is injected from <see cref="RequestOptions.SubaccountId"/>.</summary>
[Smtp2GoEndpoint("users/smtp/add")]
public sealed record SmtpUserAddRequest : IRequestValidator
{
    /// <summary>The username for SMTP authentication, 5 to 100 characters.</summary>
    ...
    void IRequestValidator.Validate(Endpoint endpoint, ICollection<string> errors)
    {
        ...
        SmtpUserRequestValidator.ValidateUsername(Username, checkLength: true, errors);
