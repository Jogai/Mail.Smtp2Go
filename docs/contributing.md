...
4. Commit, tag `v<version>` and push the tag: `git tag v1.3.5 && git push origin v1.3.5`. `publish.yml` restores in locked mode, builds and tests on Linux and Windows, packs with `ContinuousIntegrationBuild`, builds the `net48` consumer in `test/compat/` against the packed core package, verifies the package contents, pushes the three packages to nuget.org through NuGet trusted publishing (the `NuGet/login` step exchanges the GitHub OIDC token for a short-lived key; the `NUGET_USER` repository secret holds the nuget.org user name, and the repository plus `publish.yml` must be registered as a trusted publisher on that account) and creates the GitHub release from the changelog section.
